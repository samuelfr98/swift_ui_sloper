...
struct SearchView_Previews: PreviewProvider {
    ...
    @StateObject static var stubbedSearchVM: SearchViewModel = {
        ...
        return SearchViewModel(query: "Apple", sloperAPI: mock)
    }()
    
    @StateObject static var emptySearchVM: SearchViewModel = {
        var mock = MockSloperAPI()
        mock.stubbedFetchTickersCallback = { [] }
        return SearchViewModel(query: "Theranos", sloperAPI: mock)
    }()
    ...
    @StateObject static var errorSearchVM: SearchViewModel = {
        var mock = MockSloperAPI()
        mock.stubbedFetchTickersCallback = { throw NSError(domain: "", code: 0, userInfo: [NSLocalizedDescriptionKey: "An Error has occured"]) }
        return SearchViewModel(query: "Apple", sloperAPI: mock)
    }()
    ...
    static var quotesVM: QuotesViewModel = {
        ...
        return QuotesViewModel(sloperAPI: mock)
    }()
